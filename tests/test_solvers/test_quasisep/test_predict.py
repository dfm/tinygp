...
from tinygp.solvers.direct import LazyDirectSolver
...
def test_predict_mean_and_var(data, kernel, parallel):
    ...
    state = predict.precompute(solver)

    mu = kernel.matmul(X_test, X_train, beta)
    var = jax.vmap(lambda x: predict.predict_var(kernel, solver, state, x))(X_test)
...
def test_cross_matmul_gradient_finite_at_far_extrapolation():
    # ``to_general_qsm`` (hence ``kernel.matmul(X_test, X, alpha)``, the
    # conditioned mean) evaluated the transition at the clipped neighbour for
    # extrapolating rows and only masked the row afterwards, so its gradient
    # was NaN once the transition overflowed.
    rng = np_random.default_rng(0)
    X_train = jnp.sort(jnp.asarray(rng.uniform(0, 10, 20)))
    alpha = jnp.asarray(rng.normal(size=20))
    X_test = jnp.array([-5e4, 5.0, 5e4])

    @jax.grad
    def objective(scale):
        return jnp.sum(qk.Matern32(scale=scale).matmul(X_test, X_train, alpha))

    assert jnp.isfinite(objective(1.5))


@pytest.mark.parametrize("gap", [40.0, 500.0])
def test_wide_training_gap(kernel, parallel, gap):
    # Regression test: the anchors used to pull a test point back across its
    # whole training gap with an inverse transition, ~exp(+gap / scale), so a
    # gap a few dozen correlation lengths wide gave a silently wrong variance
    # (underflow) and then NaNs (overflow), in either precision.
    X_train = jnp.array([0.0, gap, gap + 1.0])
    y = jnp.array([1.0, -1.0, 0.5])
    X_test = jnp.array([0.5, gap - 1.0, gap + 0.5])
    diag = 0.1
    gp = GaussianProcess(
        kernel, X_train, diag=diag, solver=QuasisepSolver, parallel=parallel
    )
...
def test_banded_noise_falls_back_to_dense(parallel):
    # A noise model with its own quasiseparable states enlarges the factor's
    # order beyond the kernel's; the fast path must step aside (it used to
    # raise a shape error) and the result must still match the dense one.
    from tinygp.noise import Banded

    rng = np_random.default_rng(3)
    N, M = 12, 5
    X_train = jnp.sort(jnp.asarray(rng.uniform(0, 10, N)))
    y = jnp.asarray(rng.normal(size=N))
    X_test = jnp.sort(jnp.asarray(rng.uniform(-1, 11, M)))
    noise = Banded(diag=0.1 * jnp.ones(N), off_diags=0.01 * jnp.ones((N, 1)))
    kernel = qk.Matern32(scale=1.5)

    gp = GaussianProcess(
        kernel, X_train, noise=noise, solver=QuasisepSolver, parallel=parallel
    )
    cond = gp.condition(y, X_test).gp
    mu_pred, var_pred = gp.predict(y, X_test, return_var=True)

    K = kernel(X_train, X_train) + noise
    Ks = kernel(X_train, X_test)
    mu_ref = Ks.T @ jnp.linalg.solve(K, y)
    var_ref = jax.vmap(kernel.evaluate_diag)(X_test) - jnp.einsum(
        "ij,ij->j", Ks, jnp.linalg.solve(K, Ks)
    )
    assert_allclose(cond.mean, mu_ref)
    assert_allclose(cond.variance, var_ref)
    assert_allclose(mu_pred, mu_ref)
    assert_allclose(var_pred, var_ref)


def test_conditioned_covariance_includes_full_noise(parallel):
    # The lazily built conditional covariance must include the off-diagonal
    # part of a Dense test-noise model, like the other conditioning paths do.
...
def test_fast_path_survives_jit(data):
    ...
    assert isinstance(cond.solver, LazyDirectSolver)
...
def test_numpy_inputs(data):
    # The anchors gather from the training inputs with traced indices, which
    # must work when those inputs are plain numpy arrays (as in the tutorials).
    X_train, y, X_test = (np.asarray(v) for v in data)
    kernel = qk.Matern32(scale=1.5)
    gp = GaussianProcess(kernel, X_train, diag=0.1, solver=QuasisepSolver)
    cond = gp.condition(y, X_test).gp
    mu_ref, var_ref = _dense_reference(kernel, jnp.asarray(X_train), y, X_test, 0.1)
    assert_allclose(cond.mean, mu_ref)
    assert_allclose(cond.variance, var_ref)
    assert_allclose(kernel.matmul(X_test, X_train, y), kernel(X_test, X_train) @ y)
